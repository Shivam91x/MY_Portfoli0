...
// White bg pe visible colors
const STAND_COLOR = "#c8a882";      // beige/tan — stand
const MONITOR_BORDER = "#732A09";   // same brown border
const SCREEN_BG = "#1a1a2e";        // dark screen
const KB_BASE = "#e8d5c0";          // light tan keyboard base
const KB_ACCENT = "#F75454";        // red accents same
const STAIR_LINE = "#d4956a";       // warm tan stairs

export default function KeyboardSVG() {
  const [pressed, setPressed] = useState(new Set());
  const [keyColors, setKeyColors] = useState({});
  const [mouse, setMouse] = useState({ left: false, right: false });
  const [mColors, setMColors] = useState({ left: "#f472b6", right: "#818cf8" });

// File ke top pe ye add karo (useState ke saath)
const [typedKeys, setTypedKeys] = useState([]);

// Pehla useEffect hatao (jo bina preventDefault ke tha), sirf ye rakho:
useEffect(() => {
  const dn = (e) => {
    ...
    setKeyColors((p) => ({ ...p, [k]: nextColor() }));
    // Typed keys track karo (max 8)
    if (e.key.length === 1) {
      setTypedKeys((p) => [...p.slice(-7), { char: e.key, color: nextColor(), id: Date.now() }]);
    }
  };
  const up = (e) => {
    const k = e.key === " " ? " " : e.key.length === 1 ? e.key.toLowerCase() : e.key;
  ...
  const Key = ({ k }) => {
    ...
    const fs = k.fs ?? 3.6;
    return (
      <g style={{ transformOrigin: `${k.cx}px ${k.cy}px`, transform: on ? "scale(0.84) translateY(0.5px)" : "scale(1)", transition: "transform .07s cubic-bezier(.4,0,.2,1), filter .07s", filter: on ? `drop-shadow(0 0 3px ${col})` : "none" }}>
        ...
        <text x={k.cx} y={k.cy} textAnchor="middle" dominantBaseline="central" fontSize={fs} fontWeight="700" fontFamily="'Inter','Segoe UI',sans-serif" fill={on ? "#fff" : "#fff"} style={{ pointerEvents: "none", userSelect: "none", transition: "fill .07s" }}>{k.label}</text>
      </g>
    );
  };
  ...
  return (
    <div style={{
      width: "100%",
      maxWidth: "540px",
      background: "transparent",  // transparent — portfolio bg dikhega
      color: "white",
      display: "flex",  
      ...
      <div style={{ width: "100%" }}>
        <svg viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "auto", display: "block", overflow: "visible" }}>
          ...
          {/* SCREEN — dark bg so content visible */}
...
{/* SCREEN CONTENT */}
{typedKeys.length === 0 ? (
  // Idle state — blinking cursor prompt
  <g>
    ...
    {/* Prompt line */}
    <text x="88" y="110" fontSize="12" fontFamily="'Courier New',monospace" fill="#34d399" fontWeight="700">
      $
    </text>
    <text x="96" y="110" fontSize="12" fontFamily="'Courier New',monospace" fill="rgba(255,255,255,0.4)">
      type something...
    </text>

    {/* Blinking cursor */}
    <rect x="96" y="103" width="4.5" height="9" fill="#34d399" rx="0.5">
      <animate attributeName="opacity" values="1;0;1" dur="1s" repeatCount="indefinite"/>
    </rect>

    {/* Hint text bottom */}
    <text x="211.8" y="230" textAnchor="middle" fontSize="10" fontFamily="'Courier New',monospace" fill="rgba(255,255,255,0.2)">
      ↑ press any key ↑
    </text>
  </g>
) : (
  // Typed state — show characters flying in
  <g>
    <circle cx="90" cy="70" r="2.5" fill="#ff5f57"/>
    <circle cx="98" cy="70" r="2.5" fill="#febc2e"/>
    <circle cx="106" cy="70" r="2.5" fill="#28c840"/>
    ...
    <text x="88" y="110" fontSize="12" fontFamily="'Courier New',monospace" fill="#34d399" fontWeight="700">$</text>
    ...
    >
      <animate attributeName="opacity" values="1;0;1" dur="1s" repeatCount="indefinite"/>
          ...
          {/* MOUSE */}
          <path fill="none" stroke="rgba(100,80,60,0.4)" strokeWidth="1.5" strokeLinecap="round" d={`M${MCX},${MY} C${MCX},${MY-16} ${MCX+8},${MY-22} ${MCX+10},${MY-28}`}/>
          <path fill="#2a1a1a" opacity="0.6" d={`M${MX+10},${MY+2} Q${MX},${MY+2} ${MX},${MY+16} Q${MX},${MY+MH+2} ${MCX},${MY+MH+2} Q${MX+MW},${MY+MH+2} ${MX+MW},${MY+16} Q${MX+MW},${MY+2} ${MX+MW-10},${MY+2} Z`}/>
          <path fill="#3d1f1f" d={mouseBody}/>
          <path d={leftBtn} fill={mouse.left ? mColors.left : "#c03535"} style={{ transition: "fill .08s, filter .08s", filter: mouse.left ? `drop-shadow(0 0 5px ${mColors.left})` : "none" }}/>
          <path d={leftBtn} fill="white" opacity={mouse.left ? 0.15 : 0.05} style={{ pointerEvents: "none" }}/>
          <path d={rightBtn} fill={mouse.right ? mColors.right : "#c03535"} style={{ transition: "fill .08s, filter .08s", filter: mouse.right ? `drop-shadow(0 0 5px ${mColors.right})` : "none" }}/>
          ...
          <path fill="none" stroke={mouse.left ? mColors.left : mouse.right ? mColors.right : "rgba(180,100,80,0.3)"} strokeWidth="0.7" d={mouseBody} style={{ transition: "stroke .08s" }}/>
          <text x={ML_CX} y={MB_CY+1} textAnchor="middle" dominantBaseline="central" fontSize="3.2" fontWeight="700" fontFamily="Inter,sans-serif" fill={mouse.left ? "#fff" : "rgba(255,255,255,0.5)"} style={{ pointerEvents: "none", transition: "fill .08s" }}>L</text>
          <text x={MR_CX} y={MB_CY+1} textAnchor="middle" dominantBaseline="central" fontSize="3.2" fontWeight="700" fontFamily="Inter,sans-serif" fill={mouse.right ? "#fff" : "rgba(255,255,255,0.5)"} style={{ pointerEvents: "none", transition: "fill .08s" }}>R</text>
        </svg>
      </div>
    </div>
  );
